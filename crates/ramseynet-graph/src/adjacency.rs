...
///
/// For n vertices, stores n*(n-1)/2 bits packed into bytes (MSB-first within each byte).
/// Bit index for edge (i,j) where i < j: `i*n - i*(i+1)/2 + (j - i - 1)`.
...
impl AdjacencyMatrix {
    ...
    /// Create from pre-packed bits. Returns an error if the bit vector length is wrong.
...
mod tests {
    ...
    fn complement_flips_all_edges() {
        ...
        // 4 choose 2 = 6 total, 2 original edges, so complement has 4
